...
int main(void) {
	...
	canSetHWFlt(CAN_EXT, 1, CAN_RX_FIFO0, 0x0CFFFB33, 0xFFFFFF00);
	...
	while (1) {
		...
		errCodeTxCAN = canTXMessage(CAN_EXT, 0x1CE75516, 0, 8, outCanData.RawData);

		errCodeRxCAN = canRxMessage(CAN_RX_FIFO0, &pRxHeader, tstRxBuff);
		if (errCodeRxCAN == 0){
